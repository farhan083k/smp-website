...
import { MoreHorizontal, FileText, Download, ExternalLink, Plus, Edit2, Trash2, Save } from 'lucide-react';
...
import type { OtherContent } from '@/types';

interface OthersProps {
  isLoggedIn: boolean;
}

const initialOthers: OtherContent[] = [
  {
    id: 1,
...
export default function Others({ isLoggedIn }: OthersProps) {
  const [others, setOthers] = useState<OtherContent[]>(initialOthers);
  const [isEditing, setIsEditing] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editForm, setEditForm] = useState<Partial<OtherContent>>({});
  ...
  const handleEdit = (item: OtherContent) => {
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      setOthers(others.map(o => 
        o.id === editingId ? { ...o, ...editForm } as OtherContent : o
      ));
    } else {
      const newId = Math.max(...others.map(o => o.id), 0) + 1;
      setOthers([...others, { ...editForm, id: newId } as OtherContent]);
    }
    setIsEditing(false);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {others.map((item) => (
            <div
              key={item.id}
              className="mint-card p-5 group animate-fade-in"
            >
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <a
              href="#"
              className="mint-card p-4 flex items-center space-x-3 hover:shadow-lg transition-shadow"
            >
              <div className="bg-[#3498DB]/10 p-2 rounded-lg">
                <ExternalLink className="h-5 w-5 text-[#3498DB]" />
              </div>
            ...
            <a
              ...
              className="mint-card p-4 flex items-center space-x-3 hover:shadow-lg transition-shadow"
            >
              <div className="bg-[#98D8C8]/30 p-2 rounded-lg">
                <Download className="h-5 w-5 text-[#3498DB]" />
              </div>
            ...
            <a
              ...
              className="mint-card p-4 flex items-center space-x-3 hover:shadow-lg transition-shadow"
            >
              <div className="bg-[#F7DC6F]/40 p-2 rounded-lg">
                <FileText className="h-5 w-5 text-[#3498DB]" />
              </div>
        ...
        <DialogContent className="sm:max-w-lg admin-panel">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-[#2C3E50]">
              {editingId ? 'แก้ไขเนื้อหา' : 'เพิ่มเนื้อหา'}
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={deleteConfirm !== null} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent className="sm:max-w-md admin-panel">
          ...
          <p className="text-[#2C3E50]/80 py-4">
            คุณต้องการลบเนื้อหานี้ใช่หรือไม่? การกระทำนี้ไม่สามารถย้อนกลับได้
          </p>

...
// 👇 เหลือเฉพาะไอคอนที่ใช้ชัวร์ๆ เพื่อไม่ให้ Vercel บ่นครับ
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-[9999] flex flex-col p-4 md:p-8 backdrop-blur-xl overflow-hidden">
      ...
      <div className="flex-1 max-w-6xl mx-auto w-full flex flex-col md:flex-row gap-6 overflow-hidden">
        ...
        {/* แผงควบคุม (ใช้ข้อความแทนไอคอนเพื่อความชัวร์) */}
        <div className="w-full md:w-64 space-y-6 bg-white/5 p-5 rounded-2xl border border-white/10 shrink-0">
            <div className="space-y-3">
                <label className="text-xs font-bold text-gray-400 flex justify-between">
                    <span>ขยาย (Scale)</span>
                    <span className="text-blue-400">{transform.scale.toFixed(2)}x</span>
                </label>
                <input type="range" min="0.1" max="5" step="0.01" value={transform.scale} 
            ...
            <div className="space-y-3">
                <label className="text-xs font-bold text-gray-400 flex justify-between">
                    ...
                    <span className="text-white">{transform.x}px</span>
                </label>
                <input type="range" min="-800" max="800" value={transform.x} 
            ...
            <div className="space-y-3">
                <label className="text-xs font-bold text-gray-400 flex justify-between">
                    <span>บน-ล่าง (Y)</span>
                    <span className="text-white">{transform.y}px</span>
                </label>
                <input type="range" min="-800" max="800" value={transform.y} 
